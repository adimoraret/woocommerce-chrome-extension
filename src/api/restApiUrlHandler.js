...
export function getListFullUrl(resourceUrl, page, itemsPerPage, filterType, filterValue, sortBy, sortDirection){
  let url = `${SITE_URL}${resourceUrl}${AUTHENTICATION_PARAMS}&page=${page}&per_page=${itemsPerPage}`;
  if (filterType && filterValue) {
    url = `${SITE_URL}${resourceUrl}${AUTHENTICATION_PARAMS}&page=${page}&per_page=${itemsPerPage}&${filterType}=${filterValue}`;
  }
  if (sortBy) {
    url = `${url}&orderBy=${sortBy}&order=${sortDirection ? "asc" : "desc" }`;
  }
  return url;
}
